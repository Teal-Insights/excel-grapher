...
| Array formulas / spill (#284), `SUMPRODUCT`, `SUM(IF(…))` (#732) | yes / partial | `SUMPRODUCT` of covering series; aligned `SUM(IF)` / `SUMPRODUCT(IF)` as `xl_if` over range tables; spill fail closed | **port** mechanical `SUMPRODUCT` shipped ([#667](https://github.com/Teal-Insights/excel-grapher/issues/667)); aligned `SUM(IF)` shipped ([#732](https://github.com/Teal-Insights/excel-grapher/issues/732)); spill remains [#284](https://github.com/Teal-Insights/excel-grapher/issues/284). |
| Excel function coverage | `core/` + `export_runtime/` library | generic `xl_<name>` only when `runtime.py` defines it; `IF`/`CHOOSE`/`INDEX`/`MATCH`/`OFFSET`/`INDIRECT`/`EXP`/`SUM`/`SUMPRODUCT` special-cased; array `IF` via `xl_if` | **port** incrementally, fail closed otherwise. Do not embed the ctx library. |
