...
- **Normalized formulas**: each formula node has a `normalized_formula` field with sheet-qualified refs, resolved named ranges, and stripped `$` markers — ready for transpilation. It is what identifies a formula cell, the only formula text evaluation and export read, and the only text compression rewrites.
...
Extraction always stores `normalized_formula`. The raw workbook string
`Node.formula` is opt-in, because nothing in evaluation, compression, or export
...
- the original text for auditing, debugging, or display (graph exports and the
  lightweight viewer fall back to `normalized_formula` when it is absent); or
...
rewrite `normalized_formula` only, so on a compressed graph `Node.formula` still
...
(`record.formula_rewrites`) reports normalized formulas only.
