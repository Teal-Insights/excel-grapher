...
# Attach during graph build:
graph = create_dependency_graph(wb_path, ["Sheet1!D3:D100"], taco_index=True)
...
When `taco_index=False` (the default), `graph.taco_index` is `None` and graph
build behavior is unchanged. See `examples/micro_workbooks/taco_patterns.xlsx`
for RR, RF, FR, FF, and RR-Chain pattern examples, and
`examples/micro_workbooks/cross_sheet_taco_patterns.xlsx` for cross-sheet
precedents (dependent cells on one sheet, input ranges on another).
