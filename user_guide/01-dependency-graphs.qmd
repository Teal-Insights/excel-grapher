...
- **Normalized formulas**: each formula node has a `normalized_formula` field with sheet-qualified refs, resolved named ranges, and stripped `$` markers — ready for transpilation. It is what identifies a formula cell, the only formula text evaluation and export currently read, and the only text compression rewrites. Extraction also stores `formula_ast`, the parsed tree of that string (the primary in-memory formula artifact).
