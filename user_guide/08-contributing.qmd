...
Projections build artifact-specific views of the canonical graph without mutating it.
`IdentityTransitCompression().project(graph)` returns a `ProjectionResult` facade that
reads like the projected graph and carries a durable `ProjectionManifest`:

- `removed_to_replacement` public aliases
- `retained_to_collapsed_sources` and ordered `CollapsedGroup` records
- formula rewrites and node snapshots for later subgraph compression or Python refactoring

Pass the projection directly to exporters:
...
A future registry (`ExportTransformSpec`, `register_export_transform`, etc.) may follow the
same pattern as web viz layouts when config-driven projection selection is needed.
