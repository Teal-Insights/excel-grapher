...
payload = to_semantic_viz_payload(
    g, bindings, workbook="model.xlsx", blank_ranges=["Lookup!A1:C3"]
)
...
The HTML viewer lays statements out by schedule rank without overlapping labels, starts the camera on a readable window (not the full leaf-rank width), supports pan/zoom, and can hide constant/input/internal/output nodes. Large catalogs uncheck Constants on first paint. Inline HTML samples up to eight cell addresses per node; `payload.to_dict()` and `excel-grapher bindings viz --json` keep the full lists.
