...
- **Color**: primary hue maps to `module_id` (partition/community id). Formula cells stay saturated; leaf/value cells are muted; extraction `is_target` nodes are brightened. With no module overlay, all nodes share one module hue.
- **Edges**: overview prefers node-level `local_edges`; if unavailable, it falls back to module-centroid edges. Guarded edges are dashed and amber; unguarded edges are solid blue.
- **Selection**: clicking a node dims everything outside its one-hop neighborhood and brightens neighborhood nodes/edges.
