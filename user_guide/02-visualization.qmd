...
The HTML viewer lays statements out by schedule rank without overlapping labels. Sparse ranks stretch across the opening camera instead of packing at the left edge. The camera starts on a readable window (not the full leaf-rank width), and the viewer supports pan/zoom and hiding constant/input/internal/output nodes. Large catalogs uncheck Constants on first paint. Graphs whose SVG scene would exceed `SEMANTIC_VIZ_SVG_MAX_PRIMITIVES` (~20k rects and paths; Q-CRAFT stays under this, LIC-DSF does not) paint the same boxes, labels, and arrows to a canvas so the browser is not asked to composite hundreds of thousands of DOM nodes. Inline HTML samples up to eight cell addresses per node; `payload.to_dict()` and `excel-grapher bindings viz --json` keep the full lists.
