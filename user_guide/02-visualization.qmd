...
The HTML viewer lays statements out by schedule rank without overlapping labels, supports pan/zoom, and can hide constant/input/internal/output nodes. Inline HTML samples up to eight cell addresses per node; `payload.to_dict()` and `excel-grapher bindings viz --json` keep the full lists.
