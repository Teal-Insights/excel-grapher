...
The sidecar is paradigm-agnostic. **Inverted-tree** (`generate_modules(..., paradigm="inverted_tree")`)
is the recommended export for binding packages: `compute_*` functions take input arrays and return
tuples. **ctx** export still emits `set_*` / `read_*` / `Records` and remains the library default
until the [issue 662](https://github.com/Teal-Insights/excel-grapher/issues/662) gate. See
[Exporting standalone Python](06-export.qmd) for the migration and `as_records` recipe.
