...
6. Optionally add `input.setter` in the same series or a mergeable shard for the library emitter `emit_setter_function`. Generated packages validate input tensors against their declared schemas.
...
The library emitters `emit_setter_function` / `emit_compute_function` still accept **series**
and **matrix** `SeriesInput`: records or a **tidy** pandas/polars `DataFrame`. Those helpers
are not wired into `generate_modules()`. **Series** setters also accept a 1D
iterable of measure values when the binding has a single key. **Scalar** setters accept a bare
measure value, one record dict, or a one-element record list. `ctx: EvalContext` is always the
first argument.
...
See also `examples/micro_workbooks/setter_dataframe_example.py` (single-key tensor
updates) and `examples/micro_workbooks/matrix_dataframe_example.py` (matrix).
...
| **1.15.0** | Optional `input.value_map` on **scalar input** series: map public `compute_*` / `set_*` values to workbook cell needles (MATCH/IF labels). Distinct from bind kind `value_map`. |

## Input domains (1.13.0)

Declare an allowed measure domain on an input binding so generated setters and
inverted-tree `compute_*` arguments fail at the call site instead of accepting
...
Inverted-tree export checks the same domain on each required `compute_*` argument
...
```yaml
input:
  setter: {name: set_demography_scenario}
  value_map:
    High: "High "
    Medium: Medium
    Low: "Low "
