...
Single-file `generate()` is address-keyed cell functions (`make_context`, `compute_all`) for
evaluator↔export parity and teaching; it does not emit `set_*` / `read_*` / Records.
See [Exporting standalone Python](06-export.qmd) for the migration recipe.
...
5. **Export.** `CodeGenerator.generate_modules(series_bindings=..., bindings_workbook=...)` emits inverted-tree `compute_*` (explicit inputs, tuples). Single-file `generate()` is bindings-free (`make_context` / `compute_all`) and does not emit `set_*` / `read_*` / Records (see [Code export](export.qmd)).
...
are not wired into `generate()` or `generate_modules()`. **Series** setters also accept a 1D
...
Address-keyed `compute_all` is the bindings-free `generate()` entry point, not the series API.
