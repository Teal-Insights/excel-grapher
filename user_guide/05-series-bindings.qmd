...
Generated **series** setters accept `SeriesInput`: records, a 1D iterable of measure values
(single-key series only), or a **tidy** pandas/polars `DataFrame`. **Scalar** setters accept a
bare measure value, one record dict, or a one-element record list. `ctx: EvalContext` is always the
first argument.
...
Generated setters accept long/tidy `list[dict]` records:
