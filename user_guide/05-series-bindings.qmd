...
**Supported matrix shape.** Matrix setters and `compute_*` functions support **dense logical
tables**: no blank rows or columns inside the data range, with each cell-scoped key read from a
label **directly to the left** of the cell (`row_label`) or **directly above** it
(`column_header`), plus any number of whole-matrix scalar keys (`cell` / `constant`). Nested or
banded labels that require scanning beyond the immediately adjacent row/column (e.g.
country-block hierarchies) are not yet supported — see issue #287.
