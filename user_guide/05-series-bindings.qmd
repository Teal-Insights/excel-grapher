...
| **Granularity (MVP)** | One `series[]` entry per **indicator row/column** → one `set_*` function (`layout: series`) |
...
|----------|--------|-------------|
| `series` | **supported** | One indicator series × many columns (MVP) |
...
| Version | Contents |
|---------|----------|
| **1.0.0** | `series`, `scalar`; supported bind kinds above. Use for production workbooks. |
...
| **1.3.0** | Rename "row_series" to "series" to reflect that it also works with columns. no functional change
