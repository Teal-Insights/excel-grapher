...
explicit inputs and return tuples. Single-file `generate()` supports `set_*` / `read_*` /
`Records`. The context-based package exporter has been removed. See
...
5. **Export.** Package: `CodeGenerator.generate_modules(series_bindings=..., bindings_workbook=...)` emits inverted-tree `compute_*` (explicit inputs, tuples). Single-file: `generate(...)` still emits `set_*` / `read_*` / Records (see [Code export](export.qmd)).
...
4. Add `output.compute.name` (e.g. `compute_borvelia_primary_balance`). Package `compute_*` functions take input leaves and return measure tuples; use `as_records` for a Records view. Single-file `generate()` still emits Records-shaped `compute_*`.
5. `output.compute.helper` applies to single-file `generate()` only (dim-keyed internals instead of `xl_cell`). Inverted-tree packages already emit one leaf-closure function per output.
6. Optionally add `input.setter` in the same series or a mergeable shard. Setters are emitted by `generate()`, not by `generate_modules()`.
7. Pass `series_bindings` and `bindings_workbook` to `generate()` or `generate_modules()`. For packages, call `compute_*` with catalog-order arguments (or `as_records` afterward). For single-file, call `set_*` / `compute_*` with `ctx`.

### Upgrade notes (1.10.0): helper-backed compute (`generate()` only)

- **Default unchanged on single-file export:** without `output.compute.helper`, `compute_*` still fills measures via `xl_cell(ctx, address)`.
- **Helper coverage:** when a helper covers the published leaves, codegen emits dim-keyed calls such as `helper(ctx, time_period=static_record['TIME_PERIOD'])`. Partial coverage keeps `xl_cell` for uncovered leaves.
...
functions). Pass `include_compute_all=True` to `generate()` to keep it anyway.
...
`list_groups()` is emitted by `generate()` when the sidecar declares view-level `groups`.
Package exports (`generate_modules`) omit setters, readers, `make_context`, and `list_groups`.
