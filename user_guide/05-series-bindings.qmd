...
| **Granularity (MVP)** | One `series[]` entry per **indicator row** → one `set_*` function (`layout: series`) |
...
|----------|--------|-------------|
| `series` | **supported** | One indicator row × many columns (MVP) |
...
| **1.2.1** | Rename "row_series" to "series" to reflect that it also works with columns. no functional change
