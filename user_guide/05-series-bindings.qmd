...
explicit inputs and return tuples. Use `as_records(compute, result)` for a Records view.
...
5. **Export.** `CodeGenerator.generate_modules(series_bindings=..., bindings_workbook=...)` emits inverted-tree `compute_*` (explicit inputs, tuples). There is no bindings-free `generate()` path (see [Code export](export.qmd)).
...
4. Add `output.compute.name` (e.g. `compute_borvelia_primary_balance`). Package `compute_*` functions take input leaves and return measure tuples; use `as_records` for a Records view.
...
6. Optionally add `input.setter` in the same series or a mergeable shard. Setters are library emitters (`emit_setter_function`); `generate_modules()` takes catalog-order arguments instead.
7. Pass `series_bindings` and `bindings_workbook` to `generate_modules()`. Call `compute_*` with catalog-order arguments (or `as_records` afterward).
...
Package `compute_*` functions take catalog-order sequences (or scalars) and return measure
tuples. Use `as_records(compute, result)` for a tidy `list[dict]` view.
...
See also `examples/micro_workbooks/setter_dataframe_example.py` (single-key series → catalog-order
tuple) and `examples/micro_workbooks/matrix_dataframe_example.py` (matrix).
