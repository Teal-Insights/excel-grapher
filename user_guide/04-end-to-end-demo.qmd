...
def serialize_graph(graph: DependencyGraph) -> dict:
    return {
        ...
        "edges": {k: sorted(graph.get_dependencies(k)) for k in graph if graph.get_dependencies(k)},
    }
...
targets = ["S!B1"]
expected = build_graph_cache_meta_portable(
    targets, extraction_params={"load_values": True, "max_depth": 50}
)
