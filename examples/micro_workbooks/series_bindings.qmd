...
Period **4** and **5** values (`7.5` and `8.0`) are the measures you passed; `as_records` attaches `TIME_PERIOD` from `__domain__`. Read them directly as `result[4]` and `result[5]`. Package discovery is `exported.__all__` (compute names and tensor types).
