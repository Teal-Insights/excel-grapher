...
result = exported.compute_borvelia_primary_balance_out(
    exported.BorveliaPrimaryBalanceOutInputs.from_defaults(
        borvelia_primary_balance=exported.data.BORVELIA_PRIMARY_BALANCE.with_records(
            (((1,), -1.0), ((2,), -0.5), ((3,), 0.0), ((4,), 7.5), ((5,), 8.0)),
        ),
    )
)
...
Period **4** and **5** values (`7.5` and `8.0`) are the measures you passed; `as_records` attaches `TIME_PERIOD` from `__domain__`. Read them directly as `result[4]` and `result[5]`. Package discovery is `exported.__all__` (Inputs classes, compute names, and tensor types).
