...
Series I/O is an inverted-tree **package**. Package `compute_*` functions take catalog-order input tuples and return measure tuples; `as_records` zips the published domain into a Records view. There is no bindings-free `generate()` path.
