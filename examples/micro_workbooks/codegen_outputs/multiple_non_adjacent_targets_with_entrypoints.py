...
TARGET_RECORD_LAYOUT = {
    "Sheet1!C3": ("cell", ["Sheet1!C3"]),
    "Sheet1!E3": ("cell", ["Sheet1!E3"]),
}
...
def compute_e3_cell(inputs=None, *, ctx=None):
    """Compute e3_cell target cells and return Records."""
    if ctx is None:
        ctx = make_context(inputs)
    elif inputs is not None:
        warnings.warn("inputs will be ignored because ctx was provided", UserWarning, stacklevel=2)
    return _targets_to_records(ctx, TARGETS_E3_CELL, TARGET_RECORD_LAYOUT)
