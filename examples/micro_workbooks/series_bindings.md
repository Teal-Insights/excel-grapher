...
needs; export uses the overlapping leaves and skips a series
with no overlap.
...
With `require_unique_key` (the default), tensor coordinates can use
...
used as a tensor coordinate and a **record** dict (including
`OBS_VALUE` and optional attributes such as `UNIT_MEASURE`).
...
apply to `emit_compute_function`, not
`generate_modules()`.
...
dimensional keys without public `compute_*` APIs. Pipelines
