...
``examples/lic_dsf/data/lic-dsf-template-sample-exported-viz.html`` while keeping the existing
...
Use ``--full`` to rebuild from ``examples/lic_dsf/.cache/...-dependency-graph.pkl`` (slow, large
...
``examples/lic_dsf/profile_lightweight_viz.py`` (see its docstring).
"""
...
def _lic_dsf_export_targets() -> tuple[str, ...]:
    from examples.lic_dsf.extract_graph_cached import (
        EXPORT_RANGES,
        cells_in_range,
        parse_range_spec,
    )
