...
This script reuses the target list from ``examples.lic_dsf.extract_graph_cached`` but lets
...
def load_target_entries() -> list[TargetEntry]:
    from examples.lic_dsf import extract_graph_cached as cached
...
def build_dynamic_ref_config(workbook: Path) -> DynamicRefConfig:
    from examples.lic_dsf import extract_graph_uncached as uncached
...
def main() -> int:
    ...
    parser.add_argument(
        "--workbook",
        type=Path,
        default=Path("examples/lic_dsf/data/lic-dsf-template-2025-08-12.xlsm"),
