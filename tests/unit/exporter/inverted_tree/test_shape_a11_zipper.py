...
def test_vertical_lag_zipper_emits_fused_union_loop(tmp_path: Path) -> None:
    workbook = _vertical_zipper_workbook(tmp_path)
    modules = generate_inverted(workbook, _vertical_zipper_bindings())
    internals = modules["internals.py"]
    assert "cyclic formula-series" not in internals
    assert "eval_instance" not in internals
    assert "for t in range(" in internals
    pkg = load_package(modules, tmp_path, name="a11_vert")
