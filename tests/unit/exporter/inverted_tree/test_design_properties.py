...
from tests.unit.exporter.inverted_tree.test_shape_a27_indirect import (
    _literal_bindings,
    _literal_workbook,
)
from tests.unit.exporter.inverted_tree.test_shape_a27_range_aggregates import (
    ...
    range_sumproduct_if_workbook,
)
from tests.unit.exporter.inverted_tree.test_shape_a28_label_ladder import (
...
_CORPUS: list[tuple[str, Callable[[Path], Path], Callable[[], dict[str, Any]]]] = [
    ...
    ("indirect_literal", _literal_workbook, _literal_bindings),
...
def test_code_size_independent_of_constant_series_count(tmp_path: Path) -> None:
    ...
    assert "frozenset(" in small_used["data.py"]
    assert "frozenset(" in large_used["data.py"]
    small_sig_lines = [
