...
@pytest.mark.parametrize("year_key", [False, True])
def test_unrolled_matrix_reads_fixed_parameter(tmp_path: Path, year_key: bool) -> None:
    from tests.unit.exporter.inverted_tree.helpers import series_entry
    ...
    for year, column in enumerate("BCDEFG", start=2020):
        cells[f"{column}1"] = year
        for row in (2, 3):
            cells[f"{column}{row}"] = f"=IF({column}1-B{row + 3}>0,{column}1-B{row + 3},0)"
    workbook = write_workbook(tmp_path / "unrolled.xlsx", {"M": cells})
    document = bindings_document(
        series_entry(
            "payment",
            "M!B5:B6",
        ...
        series_entry("first_year", "M!B1", direction="constant"),
        series_entry("years", "M!C1:G1", layout="series", direction="constant", header_row=1),
        _matrix_entry("result", "M!B2:G3", header_row=1, direction="output"),
    )
    document["series"][-1]["layout"] = "series"
    if year_key:
        document["series"][0]["structure"]["dimensions"][0]["concept"] = "TIME_PERIOD"
        document["series"][0]["key"] = ["TIME_PERIOD"]
    pkg = load_package(
        generate_inverted(workbook, document, force_rung=2), tmp_path, name="unrolled"
    )
    assert dict(pkg.compute_result().items()) == {
        (area, year): year - payment
        for area, payment in (("a", 1), ("b", 2))
        for year in range(2020, 2026)
    }
    # Exercise the external-reference lowering used by unrolled partitions.
    from excel_grapher.exporter.inverted_tree.ast_emit import EmitContext, _emit_fused_ref
    ...
    plan = FusedPlan(scc=("result",), schedule=schedule, domain={"result": (0, 6)}, regions=())
    ctx = EmitContext(
        host=host,
        catalog=catalog,
        deps=deps["result"],
        host_index=1,
        host_cell=host.cells[1],
        index_var="t",
        prior_var=None,
        fused_mode=True,
        fused_plan=plan,
    ...
    from excel_grapher.exporter.inverted_tree.access import _iter_direct_cell_refs
    from excel_grapher.exporter.inverted_tree.deps import try_formula_ast

    refs = list(_iter_direct_cell_refs(try_formula_ast(graph, host.cells[1])))
    expression = _emit_fused_ref(catalog.get("payment").cells[0], ctx, ref=refs[1])
    assert [
        eval(expression, {"live_measure": pkg.runtime.live_measure, "payment": (1, 2), "t": t})
        for t in range(1, 6)
    ] == [1] * 5
