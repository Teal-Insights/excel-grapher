...
def test_row_view_exposes_a_one_row_range() -> None:
    ...
    assert excel.xl_sum(row) == 9.0
    assert excel.xl_index(row, None, 2) == 3.0
...
def test_column_view_resolves_only_selected_cells() -> None:
    ...
    assert excel.xl_index(column, 3, 1) == 2022.0
    assert reads == [(2022,)]
...
def test_block_view_composes_row_and_column_keys() -> None:
    ...
    assert excel.xl_index(block, 2, 1) == 6.0
