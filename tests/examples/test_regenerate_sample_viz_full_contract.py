...
def test_regenerate_sample_viz_full_contract() -> None:
    ...
    cache_path = (
        repo_root / "examples" / "lic_dsf" / ".cache" / "lic-dsf-template-2025-08-12-dependency-graph.pkl"
    )
    if not cache_path.is_file():
        pytest.skip("sample cache pickle missing; skipping full regenerate contract test")

    html_path = repo_root / "examples" / "lic_dsf" / "data" / "lic-dsf-template-sample-exported-viz.html"
    subprocess.run(
