"""Graphviz, Mermaid, and NetworkX exports show IF-guarded edges from workbooks (integration).

Builds guarded workbooks, materializes graphs, and checks dashed edges and labels
consumers expect when visualizing conditional dependency structure.
"""
...
def test_mermaid_places_guard_label_on_edge_not_node(tmp_path: Path) -> None:
    ...
    node_lines = [
        line.strip() for line in mm.splitlines() if "-->" not in line and "-.->" not in line
    ]
