"""Lightweight visualization payloads and exporters stay coherent for nontrivial graphs (integration).

Builds multi-node graphs and exercises JSON/HTML export paths, limits, and CSR-style
internals so ``to_lightweight_viz`` and related entry points remain aligned for UI consumers.
"""
...
def test_small_module_local_edges_complete() -> None:
    p = lightweight_viz_flat(
        to_lightweight_viz(_chain_graph(), max_local_nodes=100, max_local_edges=100)
    )
