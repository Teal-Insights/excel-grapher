...
BINDINGS_DOCUMENT: dict[str, Any] = {
    ...
    "series": [
        ...
        },
        {
            ...
            "layout": "series",
            "editable": True,
            "setter": {"name": "set_borvelia_year_1"},
            "structure": {
                "measure": {
                    "concept": "OBS_VALUE",
                    "dtype": "float",
...
def test_codegen_includes_setters_and_updates_inputs(workbook: Path) -> None:
    ...
    assert "def set_borvelia_year_1(" in code
    ...
    set_borvelia_year_1 = cast(
        Callable[[Any, list[dict[str, object]]], None],
        ns["set_borvelia_year_1"],
    )
    set_borvelia_year_1(ctx, [{"INDICATOR": "Real GDP growth (% per annum)", "OBS_VALUE": 3.5}])
    assert ctx.inputs["Sheet1!F4"] == 3.5
