...
def parse(formula: str) -> AstNode:
    ...
    s.skip_ws()
    if not s.eof():
        raise ParseError(formula, f"Unexpected trailing input at {s.i}")
    return node
...
def _parse_atom(s: _Scanner, original: str) -> AstNode:
    ...
    ch = s.peek()
    if ch is None:
        raise ParseError(original, "Empty formula")
    ...
    if ch == "(":
        ...
        s.skip_ws()
        if s.peek() != ")":
            raise ParseError(original, "Expected ')' after parenthesized expression")
        s.consume()
        return node
    ...
    raise ParseError(original, f"Unexpected character {ch!r} at {s.i}")
...
def _parse_quoted_sheet_ref(s: _Scanner, original: str) -> AstNode:
    ...
    if s.consume() != "'":
        raise ParseError(original, "Expected single quote")
    ...
    while True:
        ch = s.consume()
        if ch is None:
            raise ParseError(original, "Unterminated quoted sheet name")
        if ch == "'":
...
def _parse_cell_coord(s: _Scanner, original: str) -> str:
    ...
    if not col or not row:
        raise ParseError(original, f"Invalid cell coordinate at {s.i}")
...
def _parse_args(s: _Scanner, original: str) -> list[AstNode]:
    ...
    while True:
        ...
        if ch == ")":
            s.consume()
            return args
        raise ParseError(original, f"Expected ',' or ')', got {ch!r}")


def _parse_string(s: _Scanner, original: str) -> StringNode:
    if s.consume() != '"':
        raise ParseError(original, "Expected string")
    out: list[str] = []
    while True:
        ch = s.consume()
        if ch is None:
            raise ParseError(original, "Unterminated string")
        if ch == '"':
...
def _parse_number(s: _Scanner, original: str) -> NumberNode:
    ...
    try:
        ...
        raise ParseError(original, f"Invalid number literal {text!r}") from None
...
def _parse_error(s: _Scanner, original: str) -> ErrorNode:
    ...
    if err is None:
        raise ParseError(original, f"Unknown error literal {text!r}")
