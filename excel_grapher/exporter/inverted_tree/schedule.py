...
def schedule_coord(address: str, catalog: SeriesCatalog | None = None) -> int:
    ...
    When `catalog` is given and the series declares a key, the coordinate is
    the position of the full key tuple in the joined domain — a loop nest
    with `TIME_PERIOD` as the inner schedule axis (#612). When no key is
    declared (`key: []`), the coordinate is the member's expansion-order
