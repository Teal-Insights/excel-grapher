...
def materialize_precedents_for_edge(
    ...
    if meta.kind == PatternKind.rf:
        return _rf_materialize_precedents(meta, dep_key)
    if meta.kind == PatternKind.fr:
        return _fr_materialize_precedents(meta, dep_key)
...
def _rf_materialize_precedents(meta: PatternMeta, dep_key: str) -> set[str]:
    sheet, col, row = _dep_row_col(dep_key)
...
def _fr_materialize_precedents(meta: PatternMeta, dep_key: str) -> set[str]:
    sheet, col, row = _dep_row_col(dep_key)
