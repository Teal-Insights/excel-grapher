...
1. [`excel-grapher`](https://github.com/Teal-Insights/excel-grapher) - A unified engine that (a) maps and extracts cells required to compute some set of target output cells, (b) evaluates Excel formulas over that graph with Excel-equivalent semantics, and (c) exports standalone Python packages. It replaces the earlier `excel-grapher` and `excel-formula-expander` repositories.
